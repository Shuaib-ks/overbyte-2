...
import { pageHead, matchTag, matchReasons, avatar, verifiedBadge, statusBadge, aiHeader, emptyState, toast, openModal, modalHead, closeModal, kv, confirmDialog } from "../ui.js";

const filters = { cats: new Set(), maxDist: 0, maxExpiry: 0, minMatch: 0, sort: "match", q: "", vegOnly: false, context: null, bizId: null };
...
const knownMatch = (l) => l.match?.score != null && Number.isFinite(Number(l.match.score));
...
export function market() {
  if (filters.bizId !== store.get().bizId) Object.assign(filters, { cats: new Set(), maxDist: 0, maxExpiry: 0, minMatch: 0, sort: "match", q: "", vegOnly: false, context: null, bizId: store.get().bizId });
...
function marketResultsHTML(listings) {
  ...
  return `<div class="listing-grid">${rows.map((l) => {
    ...
    return `<a class="card card-hover listing-card" href="#/app/surplus/${encodeURIComponent(l.id)}" style="text-decoration:none;color:inherit">
      ...
      <div class="lc-body"><div class="row-between gap-8"><div><div class="lc-name">${esc(l.product)}</div><div class="lc-qty">${quantity(remaining(l))} ${esc(l.unit)} available</div></div>${knownMatch(l) ? matchTag(l.match, "MATCH") : ""}</div>
...
function initMarket(root) {
  const bindWatches = (container) => container.querySelectorAll("[data-watch]").forEach((button) => button.addEventListener("click", watchModal));
  const repaint = () => {
    const results = root.querySelector("#m-results");
    if (!root.isConnected || !results) return;
    const listings = selectors.marketplace(store.get());
    results.innerHTML = marketResultsHTML(listings);
    bindWatches(results);
  ...
  return `<a class="btn btn-ghost btn-sm" href="${own ? "#/app/listings" : "#/app/surplus"}" style="margin-bottom:14px">${icon("chevronLeft", 14)}Back to ${own ? "my listings" : "marketplace"}</a>
    <div class="ld-layout"><div class="col gap-20"><div class="ld-hero">${foodArt(l.product)}</div>
      <div class="card ld-info-card"><div class="row-between wrap gap-10"><div><div class="row gap-10 wrap"><h1 class="t-h1">${esc(l.product)}</h1>${statusBadge(l.status || "Active")}<span class="badge badge-neutral">${esc(l.cond)}</span></div><p class="t-body sub" style="margin-top:4px">${quantity(remaining(l))} ${esc(l.unit)} available · ${l.shelfHours > 0 ? `${fmtShelf(l.shelfHours)} until expiry` : "Expired"}</p></div><div class="ld-price">${money(l.price)}<span class="t-body muted">/${esc(l.unit)}</span></div></div>
        ...
        <div class="flex-1" style="min-width:200px"><div class="t-cap" style="margin-bottom:8px">Price &amp; handling</div>${kv([["Seller price", `${money(l.price)}/${esc(l.unit)}`], ...(l.market > 0 ? [["Seller reference price", `${money(l.market)}/${esc(l.unit)}`]] : []), ...(discount(l) ? [["Below reference", `${discount(l)}%`]] : [])])}<p class="t-sm secondary" style="margin-top:14px">${esc(l.notes || "No handling notes supplied. Confirm packaging and handling with the seller at pickup.")}</p><p class="t-xs muted" style="margin-top:12px">Listing details and expiry are supplied by the seller.</p></div></div></div>
      ...
      ${!own && l.match ? `<div class="card ai-panel" style="padding:20px"><div class="row-between">${aiHeader("OVERBYTE MATCH")}<span class="t-xs muted">Inventory-based ranking</span></div><div class="row gap-16" style="margin-top:14px">${knownMatch(l) ? `<div class="t-num-lg" style="font-size:34px;color:var(--primary-strong)">${score(l)}%</div>` : '<p class="t-sm muted">Add this product and its expected demand to see a personalized match score.</p>'}${l.match.reasons?.length ? matchReasons(l.match.reasons.slice(0, 6)) : ""}</div>${(l.match.parts || []).filter(p => !p.ok).map(p => `<p class="t-xs" style="color:var(--warning);margin-top:8px">${esc(p.detail)}</p>`).join("")}<p class="t-xs muted" style="margin-top:10px">Pickup availability must be confirmed with the seller.</p></div>` : ""}
...
export function create() {
  ...
  return `${pageHead("Create Surplus Listing", alert ? `Review ${esc(source.product)} surplus before publishing` : "You control the stock, price and pickup details", aiHeader("OVERBYTE ASSISTED"))}
    <div class="create-layout"><form class="col gap-20" id="cl-form">
      ...
      <div class="card card-pad"><div class="t-cap" style="margin-bottom:14px">Pickup &amp; expiry</div><div class="row gap-16 wrap"><div class="field flex-1" style="min-width:180px"><label for="c-pickup">Pickup window</label><input class="input" id="c-pickup" required maxlength="180" placeholder="e.g. Today, 5:00–7:00 PM" value="${esc(selectors.biz(state).pickup || "")}"/></div><div class="field" style="width:170px"><label for="c-shelf">Expires in (hours)</label><input class="input input-mono" id="c-shelf" type="number" min="0.01" max="${source.shelfHours}" step="any" required value="${Math.floor(source.shelfHours * 100) / 100}"/></div></div><p class="t-xs muted" style="margin-top:10px">A listing cannot extend the inventory batch's expiry.</p><div class="field" style="margin-top:16px"><label for="c-notes">Notes for buyers</label><textarea class="textarea" id="c-notes" maxlength="2000" placeholder="Packaging, allergens, storage and handover instructions…"></textarea></div></div>
...
function createInit(root) {
  ...
  const updateSource = () => {
    ...
    field("shelf").max = item.shelfHours;
    field("shelf").value = Math.floor(item.shelfHours * 100) / 100;
  ...
  form.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      const suggested = priceRange(item);
      const override = values.price < suggested.low || values.price > suggested.high;
      if (state.settings.priceOverrideConfirm && override && form.dataset.confirmedPrice !== String(values.price)) {
        button.disabled = false;
        confirmDialog({ tone: "primary", title: "Use your chosen price?", body: `Your price is ${money(values.price)} per ${esc(item.unit)}, outside the suggested ${money(suggested.low)}–${money(suggested.high)} range.`, confirmLabel: "Use my price", onConfirm: () => { form.dataset.confirmedPrice = String(values.price); form.requestSubmit(); } });
        return;
      }
