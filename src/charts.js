...
export function ringGauge({ value, size = 148, stroke = 11, color = "#8577ff", track = "rgba(255,255,255,.07)", label = "", sub = "", showValue = true, fs = 30 }) {
  ...
  return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}" role="img" aria-label="${label || "gauge"} ${Math.round(v)}%">
    ...
    ${sub ? `<text x="50%" y="63%" text-anchor="middle" style="font-size:10.5px;letter-spacing:.08em;text-transform:uppercase;fill:#667083;font-weight:600">${esc(sub)}</text>` : ""}
  </svg>`;
}
...
export function lineChart({ series, height = 220, yFmt = (v) => fmtNum(v), xTickEvery = 4, xUnit = "", minZero = true }) {
  ...
  if (!series.length) return `<div class="t-xs muted">No observations yet.</div>`;
  ...
  for (let t = 0; t <= ticks; t++) {
    ...
    yLabels += `<text x="${pl - 10}" y="${y + 3}" text-anchor="end" class="ch-txt">${esc(yFmt(v))}</text>`;
  }
  let xLabels = "";
  const labels = series[0].points.map((p) => p.x);
  for (let i = 0; i < n; i += xTickEvery) {
    xLabels += `<text x="${X(i)}" y="${H - 8}" text-anchor="middle" class="ch-txt">${esc(labels[i])}${esc(xUnit)}</text>`;
  }
  ...
  series.forEach((s, si) => {
    ...
    if (s.points !== undefined && s.markers !== false && s.points.length <= 16) {
      body += s.points.map((p, i) =>
        `<circle class="ch-dot" cx="${X(i).toFixed(1)}" cy="${Y(p.y).toFixed(1)}" r="3" fill="${s.color}" stroke="#0e1016" stroke-width="1.5"><title>${esc(s.name)}: ${esc(p.y)}</title></circle>`).join("");
    }
    // last value flag
    ...
    body += `<circle cx="${X(s.points.length - 1).toFixed(1)}" cy="${Y(lp.y).toFixed(1)}" r="4" fill="${s.color}"/>
      <text x="${X(s.points.length - 1).toFixed(1)}" y="${Y(lp.y).toFixed(1) - 10}" text-anchor="middle"
        style="font-family:var(--font-mono);font-size:10.5px;font-weight:600;fill:${s.color}">${esc(yFmt(lp.y))}</text>`;
  });
...
export function barChart({ data, height = 220, yFmt = (v) => fmtNum(v), seriesNames = [], colors = ["#8577ff", "#22d3ee"] }) {
  ...
  if (!data?.length) return `<div class="t-xs muted">No observations yet.</div>`;
  const max = Math.max(0, ...data.flatMap((d) => d.values)) * 1.1 || 1;
  ...
  data.forEach((d, gi) => {
    ...
    d.values.forEach((v, vi) => {
      ...
      bars += `<rect class="ch-bar" x="${x.toFixed(1)}" y="${y.toFixed(1)}" width="${w.toFixed(1)}" height="${Math.max(0, h).toFixed(1)}"
        rx="3" fill="${colors[vi % colors.length]}" opacity="${vi ? 0.85 : 1}" style="animation-delay:${gi * 60 + vi * 30}ms"><title>${esc(d.label)} · ${esc(seriesNames[vi] || "")}: ${v}</title></rect>`;
    });
    bars += `<text x="${pl + gi * groupW + groupW / 2}" y="${H - 8}" text-anchor="middle" class="ch-txt">${esc(d.label)}</text>`;
  });
...
export function sparkline(points, color = "#8577ff", w = 92, h = 30, area = true) {
  ...
  const X = (i) => (i / Math.max(1, points.length - 1)) * (w - 2) + 1;
...
export function donut(slices, size = 130, thickness = 14, centerLabel = "", centerSub = "") {
  ...
  slices.forEach((s) => {
    ...
    segs += `<circle cx="${size / 2}" cy="${size / 2}" r="${r}" fill="none" stroke="${s.color}" stroke-width="${thickness}"
      stroke-dasharray="${Math.max(0, len - 2)} ${c}" stroke-dashoffset="${off}" stroke-linecap="butt"><title>${esc(s.label)}: ${s.value}</title></circle>`;
    off -= len;
  });
  return `<svg width="${size}" height="${size}" viewBox="0 0 ${size} ${size}">
    ${segs}
    <text x="50%" y="${centerSub ? "47%" : "50%"}" text-anchor="middle" dominant-baseline="central"
      style="font-family:var(--font-mono);font-size:16px;font-weight:640;fill:#eef1f7;letter-spacing:-.03em">${esc(centerLabel)}</text>
    ${centerSub ? `<text x="50%" y="62%" text-anchor="middle" style="font-size:9.5px;letter-spacing:.07em;text-transform:uppercase;fill:#667083;font-weight:600">${esc(centerSub)}</text>` : ""}
  </svg>`;
}
...
export function flowNetwork() {
  ...
  const node = (x, y, label, sub, color) => `
    <g transform="translate(${x} ${y})">
      ...
      <text y="62" text-anchor="middle" style="font-size:10px;fill:#667083">${esc(sub)}</text>
    </g>`;
