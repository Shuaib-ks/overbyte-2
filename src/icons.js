...
export function foodArt(product, cls = "") {
  ...
  const key = ({'chicken breast':'chicken','tomatoes':'tomato','potatoes':'potato','croissants':'croissant','strawberries':'strawberry'})[raw] || raw;
  const art = ART[key] || ART.generic;
  const tile = TILES[key] || "tile-violet";
  return `<div class="food-art ${tile} ${escape(cls)}" role="img" aria-label="${escape(product)} illustration">
