...
import { fmtINR, fmtNum, esc, RISK_META } from "../util.js";
import { selectors, store } from "../store.js";
import { pageHead, riskPill, statusBadge, aiHeader, aiRec, emptyState, kv } from "../ui.js";
...
const demandBasis = item => item.demandSource === "planned" || item.dailyDemand != null ? "Your expected daily use" : item.forecastAvailable ? "Recorded consumption" : "No demand data yet";
const dateLabel = value => value ? new Date(value).toLocaleString("en-IN", { dateStyle: "medium", timeStyle: "short" }) : "Not set";
const status = item => item.shelfHours <= 0 ? statusBadge("Expired") : !item.forecastAvailable ? statusBadge("Needs demand data") : item.shortage ? statusBadge("Shortage") : riskPill(item.risk.risk);
...
function applyFilters(inventory) {
  return inventory.filter(item => {
    ...
    const itemStatus = item.shelfHours <= 0 ? "Expired" : !item.forecastAvailable ? "Needs demand data" : item.shortage ? "Shortage" : item.aiStatus;
...
export function list() {
  ...
  return `
    ${pageHead("Inventory", `${inventory.length} batches · ${fmtINR(selectors.totalInventoryValue(s))} at cost · forecasts based on your stock and recorded demand`)}
    ...
    <div class="tbl-wrap"><table class="tbl"><thead><tr><th>Product</th><th class="td-right">Qty</th><th class="td-right">Value</th><th>Expiry</th><th class="td-right">Demand to expiry</th><th>Waste risk score</th><th>AI status</th></tr></thead><tbody>
      ${rows.map(item => `<tr class="clickable" data-go="#/app/inventory/${encodeURIComponent(item.id)}">
        <td><div class="prod"><span class="p-art">${foodArt(item.product)}</span><div><a class="p-name" href="#/app/inventory/${encodeURIComponent(item.id)}">${esc(item.product)}</a><div class="p-sub">${esc(item.category)} · ${esc(item.batch || "No batch code")}</div></div></div></td>
        ...
        <td>${status(item)}<div class="t-xs muted" style="margin-top:3px">${item.shortage ? `${fmtNum(item.shortage.qty)} ${unit(item)} short` : esc(item.aiStatus || "")}</div></td>
      </tr>`).join("")}</tbody></table></div>`}
    <p class="t-xs muted" style="margin-top:12px;line-height:1.6">${icon("info", 12)} Risk scores are rule-based estimates from stock, demand and time to expiry. Record use or set expected daily use to enable forecasts.</p>`;
}
...
function stockChart(item) {
  if (!item.forecastAvailable) return emptyState({ icon: "trending", title: "A demand basis is needed", sub: "Set expected daily use or record consumption to see projected stock and surplus.", action: `<button class="btn btn-secondary btn-sm" id="set-demand">Set expected daily use</button>` });
  ...
  const points = Array.from({ length: 9 }, (_, index) => {
    ...
    return { label: `${fmtNum(hours, hours < 10 ? 1 : 0)}h`, remaining: Math.max(0, Math.round((item.qty - hours * perHour) * 100) / 100), demand: Math.round(hours * perHour * 100) / 100 };
  });
  ...
  ], yFmt: value => `${fmtNum(value, 1)} ${unit(item)}` })}<p class="t-xs muted" style="margin-top:8px">${horizon === item.shelfHours ? "Chart ends at batch expiry" : "Next 7 days shown"} · expiry ${esc(dateLabel(item.expiresAt))}. Assumes ${fmtNum(dailyRate(item), 2)} ${unit(item)}/day throughout the window.</p>`;
}
...
export function detail({ params }) {
  ...
  return `
    ...
    <div class="page-head" style="margin-bottom:20px"><div class="row gap-16">
      ...
      <div class="row gap-10 wrap"><button class="btn btn-secondary btn-sm" id="edit-item">Edit batch</button><button class="btn btn-secondary btn-sm" id="record-use">${icon("minus", 13)}Record use</button><button class="btn btn-secondary btn-sm" id="adjust-stock">${icon("plus", 13)}Update stock</button>
    ...
    <div class="dash-grid"><div class="span-4 card ai-panel" style="padding:20px">
      <div class="row-between">${aiHeader("OVERBYTE AI ANALYSIS")}<span class="t-xs muted">Rule-based forecast</span></div>
      ...
      <div class="row-between" style="margin-top:16px;padding-top:14px;border-top:1px solid var(--border)"><div><div class="t-cap">Estimated surplus</div><div class="t-num-lg" style="margin-top:4px">${forecast ? `${fmtNum(item.surplus.low)}–${fmtNum(item.surplus.high)} ${unit(item)}` : "—"}</div></div><div style="text-align:right"><div class="t-cap">Suggested price</div><div class="t-num-lg" style="margin-top:4px">${price ? `₹${price.low}–₹${price.high}` : "Set reference"}</div></div></div>
      ${item.shelfHours <= 0 ? aiRec("This batch has expired. Review its condition and record any stock disposal.") : !forecast ? aiRec("Add a demand basis to start forecasting.", `<button class="btn btn-secondary btn-sm" id="set-demand-ai">Set expected daily use</button>`) : item.shortage ? aiRec(`Projected shortage: <b>${fmtNum(item.shortage.qty)} ${unit(item)}</b> ${esc(item.shortage.byLabel || "")}.`, `<a class="btn btn-primary btn-sm" href="#/app/surplus?need=${encodeURIComponent(item.product)}">Find Surplus</a>`) : item.surplus.mid > 0 && available > 0 ? aiRec(`Review a listing for up to <b>${fmtNum(Math.min(available, item.surplus.mid))} ${unit(item)}</b> of projected surplus. You approve the quantity and price.`, `<a class="btn btn-primary btn-sm" href="#/app/surplus/new?item=${encodeURIComponent(item.id)}">Review Listing</a>`) : aiRec("No additional surplus is currently available to list. Forecasts refresh when inventory or demand changes.")}
    </div>
    <div class="span-8 col gap-20"><div class="card chart-card"><div class="ch-head"><div><div class="card-title">Inventory vs expected consumption</div><div class="t-xs muted" style="margin-top:3px">${esc(demandBasis(item))} · current stock and expiry</div></div><div class="chart-legend"><span><i style="background:#8577ff"></i>Projected stock</span><span><i style="background:#22d3ee"></i>Expected use</span></div></div>${stockChart(item)}</div>
      ...
      <div class="span-6 card card-pad"><div class="card-title" style="margin-bottom:10px">Batch details</div>${kv([
        ["Current quantity", `${fmtNum(item.qty)} ${unit(item)}`, 1], ["Reserved for listings", `${fmtNum(item.reservedQty || 0)} ${unit(item)}`, 1], ["Available quantity", `${fmtNum(available)} ${unit(item)}`, 1],
