...
function openInventoryForm(item = null) {
  ...
  const m = openModal(`
    ...
    <div class="modal-body"><form id="add-form" class="col gap-16">
      ...
      <div class="row gap-16 wrap">
        ${editing ? "" : `<div class="field flex-1" style="min-width:110px"><label for="f-qty">Quantity</label><input class="input input-mono" id="f-qty" type="number" min="0.001" step="any" required placeholder="0"/></div>`}
      ...
      <div class="row gap-16 wrap">
        ...
        <div class="field flex-1" style="min-width:150px"><label for="f-market">Reference market price (₹/unit)</label><input class="input input-mono" id="f-market" type="number" min="0.01" step="any" value="${item?.market ?? ""}" placeholder="Optional"/><span class="t-xs muted">Used for price suggestions and savings comparisons.</span></div>
      </div>
  ...
  form.addEventListener("submit", async e => {
    ...
    try {
      const payload = { product: field("product").value.trim(), category: field("category").value, cost: Number(field("cost").value), expiresAt: new Date(field("expiry").value).toISOString(), dailyDemand: field("demand").value === "" ? null : Number(field("demand").value), storage: field("storage").value, supplier: field("supplier").value.trim(), batch: field("batch").value.trim(), market: field("market").value === "" ? null : Number(field("market").value) };
      if (editing) await actions.updateInventory(item.id, payload);
      else await actions.addInventory({ ...payload, unit: field("unit").value, qty: Number(field("qty").value) });
      closeModal();
...
export function openStockMovement(item, initialKind = "consumption") {
  ...
  const m = openModal(`${modalHead("Update stock", `${esc(item.product)} · ${fmtNum(item.qty)} ${esc(item.unit)} on hand`)}
    <div class="modal-body"><form class="col gap-16" id="movement-form">
      ...
      <div class="field"><label for="movement-qty">Quantity (${esc(item.unit)})</label><input id="movement-qty" class="input input-mono" type="number" min="0.001" step="any" required placeholder="0"/></div>
